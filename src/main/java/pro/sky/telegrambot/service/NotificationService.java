...
public class NotificationService {
    ...
    public void sendNotification() {
        List<NotificationTask> tasksToNotify =
                this.repository.findByNotificationDateTimeEquals(LocalDateTime.now().truncatedTo(ChronoUnit.MINUTES));
        tasksToNotify.forEach(task -> {
                    this.telegramBot.execute(new SendMessage(task.getChatId(), task.getNotificationText()));
                });
